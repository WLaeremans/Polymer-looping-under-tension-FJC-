...
for f_index = 1:length(f_range)
    result = 0;
    f = f_range(f_index);
    F = @(x) -log(Lang_inv(x./L) ./ ((2*pi*N)^(1/2) .*sqrt(1 - (Lang_inv(x./L)./sinh(Lang_inv(x./L))).^2)) .* (sinh(Lang_inv(x./L))./Lang_inv(x./L)).^N .* exp(-N.*x./L.*Lang_inv(x./L))) - f.*x;
...
hold on
plot(f_range,log(tau),'LineWidth',2,'LineStyle', '-', 'Color', purple);
SEM = std(times19)./10;
STDs_log19 = SEM./TT19;
...
xlim([0.2 2.4])
ylim([3.6 16.7])
lgnd = legend('', '$N = 19$', '', '$N = 49$', '',  '$N = 99$', 'Theory num.', '','Location','southeast','Interpreter','latex','FontSize',14);
box on
ax = gca;
ax.FontSize = 12; 
...
z_lower_func = @(y) y;
% Define the numerical integration method
num_points = 3; % Number of points for numerical integration
% Initialize the result
result = 0;
tau = zeros(length(f_range),1);
for f_index = 1:length(f_range)
    ...
    % Define the function P_N(Rz, f)
    P_N = @(Rz) exp(beta.*f.*Rz) ./ ((4*pi*sinh(beta.*f.*b) / (beta.*f.*b)).^N) .* calculate_PN_Rz_0(Rz, N);
    
    % Define the function g(Rz)
    F = @(Rz) -log(P_N(Rz));
...
lgnd = legend('', 'Simulation', '', '', '',  '', 'Theory JH', '', 'Theory ex.','Location','southeast','Interpreter','latex','FontSize',14);
...
function PN_Rz_0 = calculate_PN_Rz_0(Rz, N)
    % Compute the P_ex value for given x and N
    % Theta is the Heaviside step function

    PN_Rz_0 = 0; % Initialize Pex

    for k = 0:N
        term = nchoosek(N, k) .* (-1).^k .* (N - Rz - 2 * k).^(N - 1) .* heaviside(N - Rz - 2 .* k);
        PN_Rz_0 = PN_Rz_0 + term;
    end

    PN_Rz_0 = PN_Rz_0 .* ((2 .* pi).^N ./ factorial(N - 1));
